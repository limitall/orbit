...
T = 0.3  # facade thickness (m)

# --- building bounds from building.json (already in Babylon X,Y,Z) ---
...
# placement maps: (h, v, depth) -> (x, y, z) world. depth 0 = outer face, +depth into building.
def make_map(kind, W):
    if kind == "front":
...
def extrude_facade(polys2d, mp):
    positions, normals, indices = [], [], []
    o = mp(0, 0, 0)
    hDir = unit(o, mp(1, 0, 0))
    vDir = unit(o, mp(0, 1, 0))
    dDir = unit(o, mp(0, 0, 1))
    outerN = [-c for c in dDir]
    innerN = list(dDir)

    def add_tri(a, b, c, n):
        base = len(positions) // 3
        for p in (a, b, c):
            positions += list(p); normals += n
    ...
    for poly in polys2d:
        ...
        # outer cap (depth 0) and inner cap (depth T)
        ...
        # side faces around exterior + holes
...
def face_is_opening(f, outer):
    ...
    if w < 0.55:                              # thin piers / witness lines -> wall
        return False
    if 0.6 <= f.area <= 9.0 and asp >= 1.0:   # window body
        return True
    if nv >= 10 and f.area <= 3.5:            # arched cap completes the window
        return True
    return False
...
for kind, fname in JOBS.items():
    ...
    opens = [f for f in faces if face_is_opening(f, outer)]
    wall = sil.difference(unary_union(opens)) if opens else sil
    polys = list(wall.geoms) if wall.geom_type == "MultiPolygon" else [wall]
    polys = [p for p in polys if p.area > 0.2]
    mp = make_map(kind, d["width"])
    pos, nor, idx = extrude_facade(polys, mp)
    out[kind] = {"positions": [round(v, 4) for v in pos],
                 "normals": [round(v, 4) for v in nor],
                 "indices": idx}
    print(f"{kind}: polys={len(polys)} tris={len(idx)//3}")

with open(BASE + "facades.json", "w") as f:
    json.dump(out, f)
print("wrote facades.json  building bounds X[%.1f,%.1f] Y[%.1f,%.1f] Z[%.1f,%.1f]" %
